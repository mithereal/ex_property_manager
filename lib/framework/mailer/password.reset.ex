defmodule Framework.Mailer.Password.Reset do
  import Swoosh.Email
  ...
  def process(user, subject \\ "Welcome to our Site!", sender \\ @sender, domain \\ @domain) do
    ...
    |> subject(subject)
    |> render_body("password_reset.html", %{username: user.name})
    |> premail()
  end
  ...
  def queue({name, email}) do
    ...
    Framework.Workers.Mailer.Password.Reset.enqueue(job, :start)
  end
end
